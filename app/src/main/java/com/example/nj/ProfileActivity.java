...
public class ProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        realname_nick = findViewById(R.id.profile_name);
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for(DataSnapshot ds : snapshot.getChildren())
                {
                    ...
                    if(email.equals(users.mail)){
                        mail.setText(users.mail);
                        realname_nick.setText(users.nickName + " (" + users.realName + " " + users.surName + ")");
                        Glide.with(getApplicationContext()).load(users.photo).into(image);
                    }
                }
            }
            @Override
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot ds : snapshot.getChildren()) {
                    ...
                    if (email.equals(users2.mail)) {
                        ...
                        map.put("photo", reference.toString());
